...
void xzhgeqz(const emlrtStack *sp, const creal_T A[9], int32_T ilo, int32_T ihi,
             int32_T *info, creal_T alpha1[3], creal_T beta1[3])
{
  emlrtStack b_st;
  emlrtStack c_st;
  emlrtStack st;
          ...
          if (muDoubleScalarAbs(b_A[jp1].re) + muDoubleScalarAbs(b_A[jp1].im) <=
            ...
            while ((!exitg2) && (j + 1 >= ilo)) {
              if (j + 1 == ilo) {
                ...
                jp1 = j + 3 * (j - 1);
                if (muDoubleScalarAbs(b_A[jp1].re) +
            ...
            if (goto70) {
              ...
              if (iiter - iiter / 10 * 10 != 0) {
                ...
                if (temp == 0.0) {
                  ...
                  ad22_re = 0.0;
                  ad22_im = temp / 0.57735026918962584;
                } else {
                  ad22_re = anorm / 0.57735026918962584;
                  ad22_im = temp / 0.57735026918962584;
                }
              ...
              st.site = &sg_emlrtRSI;
              b_xzlartg(&st, ctemp, b_ascale, &anorm, &shift);
              j = istart;
              jp1 = istart - 2;
              while (j < ilast + 1) {
                if (j > istart) {
                  ctemp_tmp = j + 3 * jp1;
                  st.site = &rg_emlrtRSI;
                  xzlartg(&st, b_A[3 * jp1 + 1], b_A[ctemp_tmp], &anorm, &shift,
                          &b_A[3 * jp1 + 1]);
                ...
                for (jp1 = j; jp1 <= ilastm; jp1++) {
                  ...
                  temp = b_A[shift_re_tmp].im;
                  tempr = b_A[shift_re_tmp].re;
                  b_ascale_re = b_A[shift_re_tmp - 1].re;
                  ascale_re = b_A[shift_re_tmp - 1].im;
                  b_A[shift_re_tmp].re =
                      anorm * tempr -
                ...
                for (i = ifirst; i <= jp1; i++) {
                  shift_re_tmp = (i + 3 * (j - 1)) - 1;
                  temp = b_A[shift_re_tmp].im;
                  tempr = b_A[shift_re_tmp].re;
                  ctemp_tmp = (i + 3 * j) - 1;
                  b_ascale_re = b_A[ctemp_tmp].re;
                  ascale_re = b_A[ctemp_tmp].im;
                  b_A[shift_re_tmp].re =
                      anorm * tempr -
                      (shift.re * b_ascale_re + shift.im * ascale_re);
                  b_A[shift_re_tmp].im =
                      anorm * temp -
                      (shift.re * ascale_re - shift.im * b_ascale_re);
                  b_A[ctemp_tmp].re = anorm * b_ascale_re +
                                      (shift.re * tempr - shift.im * temp);
                  b_A[ctemp_tmp].im =
                      anorm * ascale_re + (shift.re * temp + shift.im * tempr);
                }
